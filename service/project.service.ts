...
export async function getAllProject(): Promise<Project[]> {
  const url: string = "http://44.243.8.35:3000/projects";
...
export async function getAllSpecificProject(studentId: string): Promise<Project> {
  const url: string = `http://44.243.8.35:3000/projects/${studentId}` ;
...
export async function findByID(id: string): Promise<Project> {
  const url: string = "http://44.243.8.35:3000/projects/" + id;
  const response: Response = await fetch(url,{ cache: "no-store" });
  const project: Project = await response.json();
...
export async function updateProject(projectRequest: Project): Promise<Project> {
  const url: string = "http://44.243.8.35:3000/projects/" + projectRequest.id;
  const dto = {
...
//   const url = "http://44.243.8.35:3000/projects";
...
//   const axiosAuth = useAxiosAuth();
//   const url = `http://44.243.8.35:3000/projects/${id}`;
...
//   const axiosAuth = useAxiosAuth();
//   const url = `http://44.243.8.35:3000/projects/${projectRequest.id}`;
//   const dto = {
//     title: projectRequest.title,
//     summary: projectRequest.summary,
//     student: projectRequest.student["id"],
